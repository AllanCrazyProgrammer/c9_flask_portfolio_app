...
def add_numbers_post():
    ...
    suma_total = 0
    resta_total = float(form_text[0])
    division_total = float(form_text[0])
    multiplicacion_total = 1

    try:
        for str_num in form_text:
            suma_total += float(str_num)
            multiplicacion_total *= float(str_num)

        for str_num in form_text[1:]:
            resta_total -= float(str_num)
            division_total /= float(str_num)

        return render_template('add_numbers.html',
                               result_suma=str(suma_total),
                               result_multiplicacion=str(multiplicacion_total),
                               result_resta=str(resta_total),
                               result_division=str(division_total))
    except ValueError:
